  ...
  return Promise.resolve().then(function () {
    ...
    if (contents.length) {
      var err = "Something already exists at the destination. " +
        "`mcashbox init` and `mcashbox unbox` must be executed in an empty folder. " +
  ...
  return new Promise(function (accept, reject) {
    ...
    request(options, function (error, r) {
      if (error) {
        ...
        return reject(new Error("Mcashbox at URL " + url + " doesn't exist."));
...
function readBoxConfig(destination) {
  var possibleConfigs = [
    path.join(destination, "mcashbox.json"),
  ];
...
function cleanupUnpack(boxConfig, destination) {
  ...
  needingRemoval.push("mcashbox.json");
